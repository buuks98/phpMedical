...
<html>
    ...
    <body>
        ...
        $codi=NULL;// a NULL i el trigger ja els generarà...
        $ordre=NULL;
        $sentenciaSQL = "INSERT INTO Visites (codi,ordre, pacient, data, lloc, acompanyant) 
        	VALUES (:codi,:ordre,:pacient, TO_DATE(:data,'YYYY-MM-DD'),:lloc, :acompanyant)
        	RETURNING codi,ordre INTO :codi,:ordre";
        $comanda = oci_parse($connexio, $sentenciaSQL);
        oci_bind_by_name($comanda, ":codi", $codi);
        oci_bind_by_name($comanda, ":ordre", $ordre);
        ...
        if ($exit) {
            echo "<p>Inserida nova visita amb codi " . $codi . " i ordre " . $ordre . "</p>\n";
        } else {
